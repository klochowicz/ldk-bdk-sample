...
pub(crate) fn parse_startup_args() -> Result<LdkUserInfo, ()> {
    ...
    if env::args().len() < 3 {
        println!("ldk-tutorial-node requires 3 arguments: `cargo run electrum_url ldk_storage_directory_path [<ldk-incoming-peer-listening-port>] [bitcoin-network] [announced-node-name announced-listen-addr*]`");
        return Err(());
    }
    let electrum_url = env::args().nth(1).unwrap();
    ...
    let network: Network = match env::args().nth(arg_idx).as_deref() {
        ...
        Some(net) => {
            panic!(
                "Unsupported network provided. Options are: `regtest`, `testnet`. Got {}",
                net
            );
        }
    ...
    for line in stdin.lock().lines() {
        ...
        if let Some(word) = words.next() {
            match word {
                "help" => help(),
                "getaddress" => {
                    let address = lightning_wallet.get_unused_address().unwrap();
                    println!("{:?}", address);
                }
                "getbalance" => {
                    let balance = lightning_wallet.get_balance().unwrap();
                    println!("{:?}", balance);
                }
                "openchannel" => {
